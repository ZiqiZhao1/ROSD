...
## ProcessReflection-OOD Flat Datasets
To prepare the flat one-level dataset directories used by `ProcessReflection-OOD`, run:
```bash
python data/prepare_processreflection_ood.py \
    --targets biology,chemistry,material,physics,tooluse,aime24
```

This creates or refreshes directories such as:
```bash
