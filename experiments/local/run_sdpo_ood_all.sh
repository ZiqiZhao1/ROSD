...
# ---------------------------------------------------------------------------
# Experiment selection (skip list)
#
# Key format: "<train_target>-<backbone>", e.g. "material-Qwen3-4B"
# Put finished experiments into SKIP_EXPERIMENTS to avoid re-running them.
# ---------------------------------------------------------------------------
SKIP_EXPERIMENTS=(
  # Put finished experiments here to skip them.
  # Example:
  # "material-Qwen3-4B"
)
...
for TRAIN_TARGET in "${TRAIN_DATASETS[@]}"; do
  ...
  for BACKBONE in "${BACKBONES[@]}"; do
    COMBO_KEY="${TRAIN_TARGET}-${BACKBONE}"
    if [[ ${#SKIP_EXPERIMENTS[@]} -gt 0 ]]; then
      if in_list "$COMBO_KEY" "${SKIP_EXPERIMENTS[@]}"; then
        echo "Skipping (in skip list): ${COMBO_KEY}"
        continue
      fi
    fi
    ...
    EXPERIMENT="SDPO-OOD-train-${TRAIN_TAG}-${MODEL_TAG}"
    ...
    bash training/verl_training.sh \
      ...
      trainer.group_name=SDPO-generalization-OOD \
