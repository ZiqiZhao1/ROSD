...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode_with_teacher(self):
        """Context switch hybridengine to rollout mode using teacher (EMA) weights."""
        aggressive_empty_cache(force_sync=True)
        ...
        if teacher_module is None or teacher_module is self.actor_module_fsdp:
            # No separate teacher — fall back to student weights
            logger.warning(
                "rollout_mode_with_teacher: no separate teacher_module found; "
                "falling back to student weights. reflection_model metric will still report 'teacher'."
            )
        ...
        params = convert_weight_keys(
            params, getattr(teacher_module, "_fsdp_wrapped_module", teacher_module)
        )
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="orange", role="rollout_generate_teacher")
    def generate_sequences_with_teacher(self, prompts: DataProto):
        """Generate sequences using teacher (EMA) weights instead of student weights."""
        assert self._is_rollout
        prompts = prompts.to(get_device_id())
        ...
        if self._is_actor:
            loop = get_event_loop()
            loop.run_until_complete(self.rollout_mode_with_teacher())
            log_gpu_memory_usage("After switch to rollout mode (teacher)", logger=logger)

        with simple_timer("generate_sequences_with_teacher", timing_generate):
            output = self.rollout.generate_sequences(prompts=prompts)

        if self._is_actor:
            loop.run_until_complete(self.trainer_mode())
            log_gpu_memory_usage("After switch to trainer mode (from teacher rollout)", logger=logger)

        timing_generate_topk_ratio, timing_generate_min, timing_generate_max = topk_reduce_ratio_min_max(
            timing_generate["generate_sequences_with_teacher"]
        )
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update(
            {
