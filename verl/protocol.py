...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        if data:
            ...
            # Different workers may legitimately omit some metric keys (for example when
            # a fully-masked micro-batch skips a diagnostic computation), so we merge by
            # key union instead of requiring every metrics dict to share the same keys.
            if all_metrics:
                merged_metrics = {}
